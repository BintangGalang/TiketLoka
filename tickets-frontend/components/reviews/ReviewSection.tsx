...
import { Star, Loader2, MessageSquare, Camera, X, Image as ImageIcon } from 'lucide-react';
...
interface Review {
    ...
    rating: number;
    comment: string;
    image: string | null; // Tambah properti image
    created_at: string;
}

// Helper URL Gambar (Sama seperti di page lain)
...
export default function ReviewSection({ destinationId }: { destinationId: number }) {
    ...
    const [imageFile, setImageFile] = useState<File | null>(null); // File Asli
    const [imagePreview, setImagePreview] = useState<string | null>(null); // URL Preview
    ...
    // 2. Handle Submit Review (PAKAI FORMDATA)
        ...
        try {
            ...
            const res = await fetch('http://127.0.0.1:8000/api/reviews', {
                ...
                headers: {
                    ...
                    // Browser otomatis akan set multipart/form-data
